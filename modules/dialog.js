...
function bindEvents() {
  ...
  dom.dialogNewBtn.addEventListener('click', clearDialogHistory);
  dom.dialogAvatar.addEventListener('click', () => {
    dom.dialogPromptBox.classList.toggle('show');
  });
  ...
  dom.dialogSendBtn.addEventListener('click', async () => {
    ...
    if (!base || !key) { alert('请先设置 API'); document.getElementById('apiSettingsPage').classList.add('active'); return; }
    ...
    dom.dialogInput.style.height = 'auto';
    ...
    try {
      ...
      if (isStream) {
        ...
        if (h.length > 0 && h[h.length - 1].content === '🤔 思考中...') { h.pop();
  ...
  dom.dialogInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); dom.dialogSendBtn.click(); }
  });
...
function renderDialogMessages() {
  ...
  if (history.length === 0) {
    dom.dialogMessages.innerHTML = `<div style="color:#8e8e93;text-align:center;padding:40px 0;font-size:16px;">开始对话吧 💬</div>`;
    return;
  }
  let html = '';
  history.forEach(msg => {
...
export function openDialog(char) {
  ...
  dom.dialogAvatar.innerHTML = char.avatar ? `<img src="${char.avatar}" />` : '🧑';
  dom.dialogName.textContent = char.name || '未命名';
  dom.dialogModel.textContent = '模型: ' + (dom.modelSelect.value || '未选择');
  updateDialogPrompt();
  renderDialogMessages();
  dom.chatDialogPage.classList.add('active');
}

function updateDialogPrompt() {
  if (!currentCharId) { dom.dialogPromptBox.innerHTML = ''; return; }
  const chars = getChars();
  const char = chars.find(c => c.id === currentCharId);
  if (char && char.prompt) {
